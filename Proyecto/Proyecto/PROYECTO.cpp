...
// Keyframes
float posX = PosIni.x, posY = PosIni.y, posZ = PosIni.z, rotRodIzq = 0, rotRodDer,rotRArm,rotLArm;
...
void saveFrame(void)
{
	...
	KeyFrame[FrameIndex].rotLArm = rotLArm;
...
void interpolation(void)
{
	...
	KeyFrame[playIndex].rotIncL = (KeyFrame[playIndex + 1].rotLArm - KeyFrame[playIndex].rotLArm) / i_max_steps;
	KeyFrame[playIndex].rotIncR = (KeyFrame[playIndex + 1].rotRArm - KeyFrame[playIndex].rotRArm) / i_max_steps;
}
...
int main()
{
	...
	for(int i=0; i<MAX_FRAMES; i++)
	{
		...
		KeyFrame[i].rotIncL = 0;
		KeyFrame[i].rotIncR = 0;
	}
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		model = glm::rotate(model, glm::radians(-rotLArm), glm::vec3(1.0f, 0.0f, 0.0f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
...
void animacion()
{
		...
		if (play)
		{
			...
			else
			{
				...
				rotRArm += KeyFrame[playIndex].rotIncR;
				rotLArm += KeyFrame[playIndex].rotIncL;
				i_curr_steps++;
			}
